...
for the full arc of real work — **orient → spec → build → loop-to-green → review →
ship** — with each prompt mapped to how Claude Code actually runs (`/goal`, `/loop`,
and your `CLAUDE.md`).

## Why these are different

- **Verifiable, not vibes.** Every "done" is a mechanical, visible check (tests
...
> own output (e.g. *"`npm test` exits 0, `tsc --noEmit` is clean, and the UI
> matches the design"*). Keep the long spec in `CLAUDE.md` as background — the
...
Then in Claude Code, type `/` and they show up. Pass the task inline, e.g.
...
| `/parallel-goal` | Plan end-to-end, then execute — parallelize independent units, verify each before integrating. |
| `/maker-checker` | One pass writes, a separate fresh-eyes pass reviews; only integrate what passes. |
| `/watch` | Recurring watch — re-check a real status each cycle, act only on what changed. Pair with `/loop`. |
...
| `/ui-components` | Reusable, accessible UI components — every state covered, nothing hardcoded. |
| `/api-design` | Clean, production-ready APIs — validation, error handling, a documented contract. |

### Fix & improve
| Command | What it does |
|---|---|
| `/senior-debug` | Chase a bug to its root cause — explain it, fix it properly, prevent the class. |
| `/inherit-refactor` | Map an unfamiliar codebase, then refactor incrementally without breaking behavior. |
| `/perf-optimize` | Optimize for speed, memory, scalability — measure where the cost is before changing anything. |
...
```
/spec-from-idea  ->  /map-codebase  ->  /goal-loop  (or /tdd per unit)
       ->  /fresh-eyes-review  ->  /ship-pr
                  ^ keep /watch on a /loop to hold CI green
```
